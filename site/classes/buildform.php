  ...
  public $project;
    ...
    public function getQuickForm($id)
    {
        ...
        if (! empty($project)) {
            ...
            if ($project->published && (!$project->access || in_array($project->access, (array) qf::user()->get('roles')))) {
                if ($project->language == qf::getlang() || ! $project->language) {
                    ...
                    $form = $wpdb->get_row('SELECT * FROM '.$wpdb->prefix.'qf3_forms WHERE def=1 AND projectid = ' . ( int ) $project->id);
    ...
    {
        global $wpdb;
        return $wpdb->get_row('SELECT * FROM '.$wpdb->prefix.'qf3_forms WHERE id = ' . ( int ) $id);
    }

    public function getProjectById($id)
    {
        global $wpdb;
        $this->project = $wpdb->get_row('SELECT * FROM '.$wpdb->prefix.'qf3_projects WHERE id = ' . ( int ) $id);
    ...
    protected function userField($field, $id)
    {
        ...
        if ($type == 'phone') {
            $name = \get_user_meta(qf::user()->get('ID'),'user_phone',true);
            $type = 'tel';
        } elseif ($type == 'name') {
            $type = 'text';
        }
