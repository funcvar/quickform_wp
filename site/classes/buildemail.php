...
class qfFilds
{
    ...
    protected $db = false;
    public $config = false;
    public $qf_params = false;

    public function __construct()
    {
        $this->config = new qf_config();
        $this->qf_params = $this->config->getconfig();
    ...
    protected function checklist($name, $i, $filter = false) {
        if (isset($_POST [$name] [$i])) {
            ...
            else $value =  strip_tags($_POST [$name] [$i]);
        } else {
    ...
    protected function recaptcha($field) {
        ...
        if (qf::user()->get('guest') || ! qf::get('recaptcha_show', $this->qf_params)) {
            ...
            $data = [
                'secret' => qf::get('serverkey', $this->qf_params),
    ...
    protected function backemail($field) {
        ...
        if (qf::get('unlogged', $this->qf_params)) {
          $new = $this->findlatest('qfuseremail', FILTER_VALIDATE_EMAIL);
          if ($new) $useremail = $new;
        }
        $field->hide = 1;
        ...
        if ($useremail) {
          ...
          if ($field->value) $this->back = $useremail;
        }
    ...
    protected function qffile($field) {
        ...
        if (qf::gettask() == 'ajax.sumCustom') return $field;
        $rfiles = $_FILES;
        ...
        foreach ($rfiles ['inpfile']['name'][$i] as $k => $v) {
            if ($v) {
                ...
                if (qf::get('filesmod', $this->qf_params) && qf::get('filetoserver', $field)) {
        ...
        foreach ($data as $field) {
            ...
            if ($field->hide != 1) {
               ...
               elseif ($field->teg == 'qftabs') {
                    ...
                    if ($label) $html .= $label . ' : ' . qf::get('value', $field) . "\r\n";
                    else $html .= qf::get('value', $field) . "\r\n";
