(()=>{
  ...
  return QFcart = {
    CartBox: function() {
      ...
      var over = tag('<div class="qfcartoverlay '+cl+'"></div>').to(document.body);
      var overop = window.getComputedStyle(over).opacity || '0';
      var box = tag('<div class="qfcartform '+cl+'"><div class="qfcartclose">✕</div></div>').to(document.body);
      ...
      function changeRow(i, el) {
        ...
        QuickForm.req('option=com_qf3&task=ajax.qfcartchangerow&num=' + i + '&val=' + el.value, (res) => {
      ...
      function removeRow(i, el) {
        ...
        QuickForm.req('option=com_qf3&task=ajax.qfcartremrow&num=' + i, (res) => {
          box2.style.opacity = '0';
          box2.innerHTML = res;
    ...
    sum: function() {
      ...
      for (var u in arr) {
        ...
        html += '<div><label class="qf3label">' + pricebox.dataset.text_price_total + '</label><span class="qfpriceinner">' + QFcart.round(arr[u]) + '</span><span class="qfunitinner">' + u + '</span></div>';
      }
    ...
    round: function(value) {
      return QuickForm.strPrice(Number(Math.round(value + 'e' + qf_cart_fixed) + 'e-' + qf_cart_fixed).toFixed(qf_cart_fixed), qf_cart_format);
    },
    ...
    activateDiscaunt: function(box) {
      ...
      if (promocodbtn) {
        ...
        promocodbtn.onclick = function() {
          if (this.className != 'enabled' && !this.form.checkValidity()) {
            var sub = tag('<input type="submit" style="display:none">').to(this.form);
            sub.click();
            sub.remove();
      ...
      function setdiscount(disc, abs) {
        ...
        tag('<div class="finalpricerow"><label>' + label + '</label><span class="qfpriceinner">-' + QFcart.round(discount) + '</span><span class="qfunitinner">' + unit + '</span><input name="qfprice[]" type="hidden" value="-' + discount + '" data-unit="' + unit + '"></div>').to(resbox);
      ...
      function doDiscounts() {
        ...
        var price = priceinput.value;
        var ar, disc, abs, arr = qf_cart_discount.match(/([^&%]+[&%])/g)||[];
      ...
      function getPromocod(btn) {
        QuickForm.req('option=com_qf3&task=ajax.qfcartpromocod&cod=' + promocodinp.value, (res) => {
          if (res.length > 1) {
    ...
    attachmentbox: function(indx) {
      const box = tag('<div class="atch_box"><div class="qfcartclose">✕</div><div>').to(document.body);
      ...
      QuickForm.req('option=com_qf3&task=ajax.showAttachmentBox&num=' + indx, (res) => {
    ...
    activateAttachmentbox: function(box, iii) {
      ...
      $('.atch_link_more a', box).onclick = function() {
        this.insertAdjacentHTML('beforebegin', '<div class="atch_link"><input type="text" placeholder="' + $('.atch_link input', box).placeholder + '"></div>');
        checkLinks();
        return false;
      }
      checkLinks();
      ...
      function del_old_img(el) {
        ...
        if (spl) {
          ...
          QuickForm.req('option=com_qf3&task=ajax.attachment_del_img&name=' + spl + '&num=' + iii, (res) => {
            prel.remove();
            if (res == 'yes') {
              imgbox.remove();
            } else {
      ...
      function displayFiles(files) {
        ...
        Object.keys(files).forEach((k) => {
          ...
          if (!accept || accept.includes(ext)) {
            var ibox = tag('<div class="imgtbox_new"></div>').to($('.filelisting', box));
            var ateg = tag('<div class="file_name">' + files[k].name + ' (' + (files[k].size / 1024 / 1024).toFixed(2) + 'M)</div>').to(ibox);
            var del = tag('<span class="imgtdel">✕</span>').to(ibox).onclick = function() {
              ibox.remove();
    ...
    cartnext: function() {
      var fl, forms = $$('#qf_delivery form, #qf_payment form');
      forms.forEach((el) => {
        if (!el.checkValidity()) {
          fl = 1;
          return el.submit();
    ...
    cartsubmit: function(form) {
      var fl, dat, n = 0;
      var forms = $$('.qf_cart_foot_l .qf3form form, #qf_contacts .qf3form form');
    ...
    animateMiniCart: function(res) {
      ...
      opacity(module, 0, 300, (module) => {
        ...
        this.animateShuttle(res.replace(/<span class="qf_cart_incart".*<\/span>/g, ''));
